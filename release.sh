...
########################################################################
## OS - /usr/lib
echo "Creating 'OS - /usr/lib' Debian package..."
SPLIT_DEB_USRLIB=${NAME}-os-usrlib
mkdir -p ${BASEDIR_TMP}

# copy over the OS /usr/lib files
rsync -axHAWX --numeric-ids --verbose --relative ${BASEDIR}/./${MEDIA_RPI_PATH}/nfs/usr/lib ${BASEDIR_TMP}
...
Version: ${VERSION_LONG}
Maintainer: ${MAINTAINER}
Description: RPi 'os - /usr/lib' sub-package: contains the large /usr/lib OS files
Architecture: ${ARCH}
Priority: optional
Replaces: ${NAME} (<< 2.2.0)
Breaks: ${NAME} (<< 2.2.0)
...
rm -rf ${BASEDIR_TMP}
echo "Creating 'OS - /usr/lib' Debian package... Done"
...
rsync -axHAWX --numeric-ids --verbose --exclude="/*/" ${BASEDIR}/${MEDIA_RPI_PATH}/ ${BASEDIR_TMP}
...
Architecture: ${ARCH}
Priority: optional
Depends: ${SPLIT_DEB_BOOT}, ${SPLIT_DEB_USRLIB}, ${SPLIT_DEB_OTHER}
EOL
